...
/// The garden as a grid of tiles: what `PlotView` replaced.
///
/// Kept, and reachable only from the developer section, because the plot is the
/// screen that has to be judged on a real garden on a real phone and this is the
/// only thing there is to judge it against. It is not dead code yet; it is the
/// control.
///
/// **It is still compiled in a Release build, on purpose.** Putting it behind
/// `#if DEBUG` would take `tile.caption` out of the string catalogue, and a
/// catalogue entry in this app is forty-two translations. The cost of leaving it
/// is a view nothing presents; the cost of removing it is measured in
/// commissions.
